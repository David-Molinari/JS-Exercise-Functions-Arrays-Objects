...
function makePersonObject(id1, name1, email1) {
  return {id:id1, name:name1, email:email1};
}
...
function getCarInfoByIndex(inventory, index) {
  let theRow = inventory[index];
  return 'This is a ' + theRow.car_make + ' ' + theRow.car_model;
}
...
function getLastCarInfo(inventory) {
  let invenNumba = inventory.length;
  let indexNumba = invenNumba - 1;
  let lastCar = inventory[indexNumba];
...
function getCarInfoById(inventory, id) {
  let theCarIndy = id - 1;
  let theCarCar = inventory[theCarIndy];
  return 'This is a ' + theCarCar.car_make + ' ' + theCarCar.car_model;
}
...
function carMaker(num) {
  ...
  function drive(distance) {
    let newRead = retObj.odometer += distance;
    let retObj = newRead;
  }
}
return retObj
